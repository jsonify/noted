...
Use dynamic keywords that auto-update based on the current date:

**TODAY:**
```
Query: from:TODAY to:TODAY authentication
Result: Notes from today containing "authentication"
```

**YESTERDAY:**
```
Query: from:YESTERDAY meeting
Result: Yesterday's notes about meetings
```

**LAST N DAYS:**
```
Query: from:LAST 7 DAYS tag:bug
Result: Bugs from the last 7 days
```

```
Query: from:LAST 30 DAYS security
Result: Security notes from the last 30 days
```

**THIS WEEK:**
```
Query: from:THIS WEEK tag:meeting
Result: Meeting notes from Sunday to today
```

**THIS MONTH:**
```
Query: from:THIS MONTH refactor
Result: Notes about refactoring this month
```

**THIS YEAR:**
```
Query: from:THIS YEAR tag:project-alpha
Result: Project Alpha notes from this year
```

These keywords are **case-insensitive** and perfect for saved searches that stay current!
